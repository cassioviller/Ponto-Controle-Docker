...
- **Toggle por funcionário `he_100_acima_2h`** (default `true`): quando `false`, em dias com `tipo_dia='normal'` todo o excedente (entrada/saída além da jornada) vai para HE 60% — sem o teto de 2h — e HE 100% fica zerada. Outros tipos de dia não são afetados. Editado em "Gestão de Funcionários" (seção "Configurações de Trabalho"). Ao alternar este toggle, `PUT /api/funcionarios/:id` faz backfill transacional de todos os registros do funcionário com `tipo_dia='normal'` e devolve `registros_recalculados: N` (mostrado como toast no front).
- **Override de intervalo por dia**: o campo `intervalo` enviado em `POST /registros` (e o intervalo digitado no modal da Folha Individual quando ambas as pontas do almoço estão vazias) tem precedência sobre `jornada_padrao.intervalo_padrao`. `"00:00"` é honrado como "sem intervalo" (uso de `??` em vez de `||` em `calcFromTipoDia`/`calcFromJornada`).
